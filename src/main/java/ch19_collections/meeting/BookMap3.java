            ...
            switch (option){
                ...
                case 2 :
                    ...
                    if(bookInventory.containsKey(bookTitle)) {
                        System.out.println("입고되는 수를 입력하세요 >>> ");
                        int tmp = in.nextInt();
                        int stock = bookInventory.get(bookTitle) + tmp;//해당 키의 스톡을 가져오는건가
                        System.out.println(bookTitle + "도서가 "+stock+" 권으로 추가되었습니다.");
                        bookInventory.put(bookTitle,stock);
                    }
                ...
                case 3:
                    ...
                    bookTitle = in.nextLine();
                    if(bookInventory.containsKey(bookTitle)) {
                        System.out.println("출고되는 수를 입력하세요 >>> ");
                        int tmp = in.nextInt();
                        if(tmp>bookInventory.get(bookTitle)) {
                            System.out.println("재고가 부족합니다.");
                            break;
                        }
                        int stock = bookInventory.get(bookTitle) - tmp;
                        System.out.println(bookTitle + "도서가 "+stock+" 권으로 감소되었습니다.");
                        bookInventory.put(bookTitle,stock);
                    }
                    else System.out.println("해당 제목의 도서가 존재하지 않습니다.");
                    break;
