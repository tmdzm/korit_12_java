...
프로젝트 설정 관련 Name : korit_12_java_000 Location : c:여러분영어이름폴더 Build system : Gradle Gradle DSL : Groovy

JDK : download JDK 선택 -> version : 17 -> vendor : Amazon corretto

-> 프로젝트 생성 후에 build.gradle -> 코드입력 -> 코끼리 표시 클릭 -> 이후에 실행해봤을 때 정상으로 나오면 됩니다.

-> C:여러분영어이름폴더 -> 우클릭 -> 새로 만들기 -> 폴더 -> learning_logs

-> learning_logs shift + 우클릭 -> open git bash here -> code .
...
Conversion(형변환) : 특정 변수(객체)를 다른 자료형으로 변환시키는 것.
upcasting : 특정 변수(객체)를 더 큰 용량의 자료형으로 변환시키는 것.

암시적(implicit)으로 이루어짐 : 작은 용량에서 큰 용량으로 변환시킬 때는 데이터의 값이 변질되지 않기 때문에 직접적으로 바뀔 자료형을 명시하지 않아도 됨.
downcasting : 특정 변수(객체)를 더 작은 용량의 자료형으로 변환시키는 것.
